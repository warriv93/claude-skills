...
3. **Security pass:** Run **`/security-review`** via subagent (**Cheap Model**) over feature diff (`git diff --stat` first). Place static rules at top of prompt; mandate **`NO_PREAMBLE`**. Treat criticals as blocking. Log findings to `.deep-plan/state-findings.md`.
...
7. **Second Code Quality Gate (Diff-scoped):** Re-run `code-review` subagents on **Strong Model**, strictly scoped to diff introduced during Phase 4 fixes (`git diff <phase-3-end-sha>..HEAD`). Use prompt cache prefix alignment (`static instructions top`) and **`NO_PREAMBLE`**. Log findings to `.deep-plan/state-findings.md`.
